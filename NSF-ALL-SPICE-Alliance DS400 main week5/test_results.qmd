...
probability_test_positive <- 0.0112
```
...
```{r}
(probability_disease * probability_positive_result_given_disease)/probability_test_positive
```
...
##If you test positive again, the prior probability in Bayes' Theorem needs to be updated to reflect the first positive test result. This new prior increases the likelihood of having the disease after the second positive test. As a result, the probability of actually having the disease will be higher with two positive tests compared to just one.
